...
**Transformations**: 
- Original file is converted from JSON to YAML and from Swagger 2.0 to OpenAPI 3.0.1 with the following tool: https://editor.swagger.io/
- File is pruned to only include the endpoints and schemas relevant to GraphGroup resource.

**List of change made to the OpenAPI Specification (OAS)**:
- Created new schema `GraphGroupCreationContextUnified` for the request body of the `create` operation.
- Commented out the `GET /{organization}/_apis/graph/groups` endpoint (for the `findby` action) since a plugin endpoint is used instead to provide better filtering capabilities.
- Commented out the `PATCH /{organization}/_apis/graph/groups/{groupDescriptor}` endpoint since updating GraphGroups is done via JSON Patch operations and KOG does not currently support it.
- Changed the status code for the `create` operation from `200` to `201` since the Azure DevOps REST API actually returns a `201 Created` status code when a GraphGroup is created successfully.
- Changed the status code for the `delete` operation from `200` to `204` since the Azure DevOps REST API actually returns a `204 No Content` status code when a GraphGroup is deleted successfully.
- Commented out the `oauth2` security scheme since it not supported by OASGen Provider and Rest Dynamic Controller while authentication is handled at the root level of the file and with basic auth scheme.
- Commented out `x-ms-*` items as they are specific to Microsoft internal tooling.
- Commented out the `components.parameters` section at root level as it just to inform about api versions.
...
**Transformations**: 
- Original file is converted from JSON to YAML and from Swagger 2.0 to OpenAPI 3.0.1 with the following tool: https://editor.swagger.io/
- File is pruned to only include the endpoints and schemas relevant to User resource.

**List of change made to the OpenAPI Specification (OAS)**:
- Created new schema `GraphUserCreationContextUnified` for the request body of the `create` operation.
...
**Version**: 7.2-preview.2
**Original specification file**:
- Link: https://github.com/MicrosoftDocs/vsts-rest-api-specs/blob/master/specification/distributedTask/7.2/taskAgent.json
- Permalink: https://github.com/MicrosoftDocs/vsts-rest-api-specs/blob/eca8bf9e190633aa1c3c2877b74c5a13aa9d4b89/specification/distributedTask/7.2/taskAgent.json

**Transformations**: 
- Original file is converted from JSON to YAML and from Swagger 2.0 to OpenAPI 3.0.1 with the following tool: https://editor.swagger.io/
- File is pruned to only include the endpoints and schemas relevant to VariableGroup resource.

**List of change made to the OpenAPI Specification (OAS)**:
- Added plugin endpoints for `get`, `findby` and `delete` operations to handle secret variable restoration and organisation-scoped delete operation.
- Added `type: object` for `IdentityRef` schema to be aligned with schemas generated by swag tool (plugin endpoints are generated with swag).
- Commented out the `oauth2` security scheme since it not supported by OASGen Provider and Rest Dynamic Controller while authentication is handled at the root level of the file and with basic auth scheme.
- Commented out `x-ms-*` items as they are specific to Microsoft internal tooling.
- Commented out the `components.parameters` section at root level as it just to inform about api versions.
