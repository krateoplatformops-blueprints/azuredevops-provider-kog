...
**Starting point**: `PolicyConfiguration` resource managed by Azure DevOps Provider (classic).
...
Note that the `PolicyConfiguration` resource is a non-namespaced resource in the context of Azure DevOps Provider (classic), while it is a namespaced resource in the context of Krateo Azure DevOps Provider KOG (you can check this by running the following command):
```sh
...
Note that the `projectRef` field has been replaced with `projectId`, which can be either the `ID` or the `name` of the project in the case of the spec of the `GitRepository` resource.
