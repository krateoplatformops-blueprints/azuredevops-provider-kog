...
This document outlines the process for releasing new versions of the components within this repository. The project uses a **component-based tagging strategy**, where each independent part (plugin, resource blueprint, main ("umbrella") blueprint) is versioned and released separately.
...
| ----------------------- | ----------------------------------- | ---------------------------------- |
| Plugin Source           | `pipeline-plugin/1.0.3`         | `plugins-source-tag.yaml`          |
| Resource Blueprint      | `pipeline-blueprint/1.0.3`      | `resource-blueprint-tag.yaml`      |
...
**Goal:** Release a new feature that requires changes to the `pipeline` plugin, its blueprint, and the main umbrella chart.
...
- **Action:** Tag and push the new version for the `pipeline-plugin`.
- **Command:**
  ```sh
  git tag pipeline-plugin/1.2.0
  git push origin tag pipeline-plugin/1.2.0
  ```
- **Result:** The `plugins-source-tag` workflow runs, publishing the `pipeline-plugin:1.2.0` container image to the GitHub Container Registry.
...
- **Action:** Tag and push the new version for the `pipeline-blueprint`.
- **Command:**
  ```sh
  git tag pipeline-blueprint/1.2.0
  git push origin tag pipeline-blueprint/1.2.0
  ```
- **Result:** The `resource-blueprint-tag` workflow runs. It will **automatically**:
  1.  Find the `pipeline-plugin:1.2.0` image you just published (latest tag).
  2.  Set the chart's `version` to `1.2.0` and its `appVersion` to `1.2.0` (appVersion reflects the plugin image version).
  3.  Publish the new `azuredevops-provider-kog-pipeline` chart to the Helm repository.
...
- **Command:**
  ...
  ```
- **Result:** The `main-blueprint-tag` workflow runs. It will **automatically**:
  1.  Find the `azuredevops-provider-kog-pipeline:1.2.0` chart you just published.
  ...
  4.  Publish the new `azuredevops-provider-kog` chart to the Helm repository.
